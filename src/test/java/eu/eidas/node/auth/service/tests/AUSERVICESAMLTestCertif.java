...
/**
 * testing SamlEngine when enforcing certificate validity or self signed certificated
 */
public class AUSERVICESAMLTestCertif {
    ...
    public static void runBeforeClass() throws Exception {
        ...
        CONFIGS.setProperty(
                EIDASErrors.COLLEAGUE_REQ_INVALID_COUNTRYCODE.errorCode(), "002001");
        CONFIGS.setProperty(
    ...
    public void testProcessAuthenticationRequest() {
        ...
        CONFIGS.setProperty(SAMLEngineSignI.SELF_SIGNED_PROPERTY, "true");
        auserviceutil.setConfigs(CONFIGS);
        try {
            auservicesaml.processAuthenticationRequest(saml, session,
                    TestingConstants.USER_IP_CONS.toString());
            assertTrue("processAuthenticationRequest should throw and Exception since the certificates are self signed", false);
        }catch(InternalErrorEIDASException exc){
            assertTrue(exc.getCause() instanceof EIDASSAMLEngineException);
            assertTrue(exc.getCause().getMessage().contains("Self signed certificates are not allowed"));
        }
    }
